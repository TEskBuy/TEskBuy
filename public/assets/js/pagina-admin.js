...
(function () {
  ...
  function verPainel() {
    ...
    api.get('/admin/painel').then(function (r) {
      ...
      // os cartões do marketplace levam ao separador certo
      document.querySelectorAll('[data-ir]').forEach(function (b) {
        b.addEventListener('click', function () {
          var alvo = conteudo.querySelector('[data-vista="' + b.getAttribute('data-ir') + '"]');
          if (alvo) alvo.click();
        });
      });
    }).catch(erroPainel);
  ...
  /* ── contadores do marketplace ───────────────────────────── */
  function kpisMarketplace(m) {
    if (!Object.keys(m).length) return '';

    function kpi(rotulo, valor, alerta, destino) {
      var corpo = '<p>' + rotulo + '</p><p class="v mono">' + valor + '</p>';
      return destino
        ? '<button class="kpi' + (alerta && valor ? ' alerta' : '') + '" data-ir="' + destino +
          '" style="text-align:left;border:1px solid var(--linha);cursor:pointer;font:inherit;color:inherit">' +
          corpo + '</button>'
        : '<div class="kpi' + (alerta && valor ? ' alerta' : '') + '">' + corpo + '</div>';
    }

    return '<h3 style="margin:26px 0 12px">Marketplace</h3>' +
      '<div class="kpis">' +
        kpi('Utilizadores', m.utilizadores) +
        kpi('Novos (7 dias)', m.utilizadores_novos_7d) +
        kpi('KYC pendentes', m.kyc_pendentes, true, 'candidaturas') +
        kpi('Pedidos de vendedor', m.candidaturas_vendedor, true, 'candidaturas') +
        kpi('Pedidos de afiliado', m.candidaturas_afiliado, true, 'candidaturas') +
        kpi('Vendedores aprovados', m.vendedores_aprovados) +
        kpi('Afiliados aprovados', m.afiliados_aprovados) +
        kpi('Produtos publicados', m.produtos_publicados) +
        kpi('Por moderar', m.produtos_por_moderar, true, 'produtos') +
        kpi('Parcerias por analisar', m.parcerias_pendentes, true, 'parcerias') +
        kpi('Denúncias abertas', m.denuncias_pendentes, true, 'denuncias') +
        kpi('Tickets abertos', m.tickets_abertos, true, 'tickets') +
        kpi('Encomendas canceladas', m.encomendas_canceladas) +
        kpi('Reembolsos', m.encomendas_reembolsadas) +
        kpi('Comissões de afiliados', ui.kz(m.comissoes_afiliados)) +
      '</div>';
  }

  /* ── auditoria ───────────────────────────────────────────── */
  function verAuditoria(entidade) {
    esqueleto(2);
    var filtro = entidade === undefined ? '' : entidade;
    var ENTIDADES = ['', 'produto', 'empresa', 'candidatura', 'parceria', 'denuncia', 'encomenda'];

    api.get('/admin/auditoria', { entidade: filtro || undefined, limite: 100 })
      .then(function (r) {
        var itens = r.dados || [];

        document.getElementById('painel-admin').innerHTML =
          '<h1 style="margin-bottom:4px">Auditoria</h1>' +
          '<p class="silenciado pequeno" style="margin-bottom:16px">' +
            'Quem fez o quê, e quando. Serve para investigar qualquer problema.</p>' +
          '<div class="pilulas" style="margin-bottom:18px">' +
            ENTIDADES.map(function (e) {
              return '<button class="pilula' + (e === filtro ? ' activa' : '') + '" data-afiltro="' + e + '">' +
                (e ? e.charAt(0).toUpperCase() + e.slice(1) : 'Tudo') + '</button>';
            }).join('') +
          '</div>' +
          (itens.length
            ? '<div class="cartao">' +
                itens.map(function (a) {
                  return '<div class="linha-flex" style="justify-content:space-between;gap:12px;' +
                    ...
                    '<div style="min-width:0">' +
                      '<p class="pequeno mono">' + ui.escapar(a.accao) + '</p>' +
                      '<p class="pequeno silenciado">' + ui.escapar(a.autor) + '</p>' +
                    '</div>' +
                    '<span class="pequeno silenciado" style="flex-shrink:0">' +
                      ui.data(a.quando, true) + '</span>' +
                  '</div>';
                }).join('') +
              '</div>'
            : '<div class="cartao-vazio"><h3>Sem registos</h3>' +
              '<p class="silenciado">Ainda não há actividade deste tipo.</p></div>');

        document.querySelectorAll('[data-afiltro]').forEach(function (b) {
          b.addEventListener('click', function () { verAuditoria(b.getAttribute('data-afiltro')); });
        });
      })
      .catch(erroPainel);
  }
  ...
  function abrir() {
    ...
    if (vista === 'auditoria' && eu.papel === 'admin') return verAuditoria();
    return verPainel();
  }
